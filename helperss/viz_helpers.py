...
import configs
import model
from myhelpers.data_helpers import get_all_embeddings_or_data, restart_label_index_from_zero, create_tool_idx_list
...
def viz_data(trans_cls, encoder: model.encoder or None, data_dim=None,
             ...
             trail_list=configs.trail_list, viz_l2_norm=configs.viz_share_space_l2_norm):
...
def _viz_embeddings(embeds: np.ndarray, labels: np.ndarray, tool_labels: list, loss_func: str = configs.loss_func,
    ...
    logging.debug(f"viz_shared_latent_space.. viz_l2_norm: {viz_l2_norm}")
...
def _viz_classifier_boundary_on_2d_embeddings(
    ...
    plt.legend()
    plt.grid(True)  # TODO: save this figure
    # plt.tight_layout()
    if viz_l2_norm:
...
def plot_learning_progression(record, type, TL_margin, loss_func, sincere_temp, lr_classifier, lr_encoder,
    ...
    logging.debug(f"plot_learning_progression for {type}: {save_name}...")
