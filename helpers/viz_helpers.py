...
import configs
import model
from helpers.data_helpers import stack_input_data, get_all_embeddings, make_new_labels_to_curr_obj, \
    restart_label_index_from_zero, create_tool_idx_list
...
def viz_input_data(data, shared_only: bool, test_only: bool, loss_func_name: str = configs.loss_func,
                   ...
                   assist_tool_list=configs.assist_tool_list,
    ...
    """ data order: source -> assist_train -> target_train-> assist_test-> "target_test """
    ...
    if shared_only:
        ...
        obj_list = old_object_list
        new_object_list = []
    elif test_only:
        logging.info("visualize the TEST object set in input data space...")
        obj_list = new_object_list
        old_object_list = []
    else:
        logging.info("visualize the ALL object set in input data space...")
        obj_list = old_object_list + new_object_list

    source_label_len, assist_label_train_len, assist_label_test_len = 0, 0, 0
    target_label_train_len, target_label_test_len = 0, 0
    X_array, Y_array = stack_input_data([], [], behavior_list, source_tool_list, obj_list, data=data)
    source_label_len += len(X_array)
    prev_len = len(X_array)
    if not test_only:
        X_array, Y_array = stack_input_data(X_array, Y_array, behavior_list, assist_tool_list, old_object_list,
                                            data=data)
        assist_label_train_len += len(X_array) - prev_len
        prev_len = len(X_array)
    if not shared_only:
        X_array, Y_array = stack_input_data(X_array, Y_array, behavior_list, assist_tool_list, new_object_list,
                                            data=data)
        assist_label_test_len += len(X_array) - prev_len
        prev_len = len(X_array)

    if not test_only:
        X_array, Y_array = stack_input_data(X_array, Y_array, behavior_list, target_tool_list, old_object_list,
                                            data=data)
        target_label_train_len += len(X_array) - prev_len
        prev_len = len(X_array)
        if shared_only:
    ...
    if not shared_only:
        X_array, Y_array = stack_input_data(X_array, Y_array, behavior_list, target_tool_list, new_object_list,
                                            data=data)
        target_label_test_len += len(X_array) - prev_len
        if test_only:
    ...
    Y_curr_labels = np.array([obj_list.index(obj) for obj in Y_objects])  # reassign labels based on all_obj_list

    tool_labels = create_tool_idx_list(source_label_len, assist_label_train_len,
        assist_label_test_len, target_label_train_len, target_label_test_len)

    subtitle = f"target tool: {target_tool_list}, source tool: {source_tool_list} \nassist tool: {assist_tool_list}"
    viz_embeddings(loss_func=loss_func_name, obj_list=obj_list,
                   embeds=X_array, labels=Y_curr_labels, save_fig=True,
                   tool_labels=tool_labels, title=plot_title, subtitle=subtitle)
...
def viz_emb_in_shared_space(trans_cls, encoder: model.encoder, viz_l2_norm=True):
    encoder.l2_norm = viz_l2_norm   # option to expand the unit circle for more details
    all_emb, all_labels, _ = get_all_embeddings(trans_cls, encoder)
    # all_emb order: source_old, source_new, assist_old, assist_new, target_old, target_new
    source_data_train, source_label_train = all_emb[0], all_labels[0]
    source_data_test, source_label_test = all_emb[1], all_labels[1]
    assist_data_train, assist_label_train = all_emb[2], all_labels[2]
    assist_data_test, assist_label_test = all_emb[3], all_labels[3]
    target_data_train, target_label_train = all_emb[4], all_labels[4]
    target_data_test, target_label_test = all_emb[5], all_labels[5]

    # viz train data on shared latent space
    embeds = np.vstack([source_data_train, source_data_test, assist_data_train, target_data_train])
    ...
    viz_embeddings(embeds=embeds, labels=labels, tool_labels=tool_labels, obj_list=configs.all_object_list, save_fig=True,
                   title=f"Shared Space-Loss-{configs.loss_func} \n (Encoder Training Object)",
    ...
    embeds = np.vstack([source_data_test, assist_data_test, target_data_test])
    ...
    viz_embeddings(viz_l2_norm=viz_l2_norm, embeds=embeds, labels=labels, tool_labels=tool_labels, obj_list=configs.new_object_list,
                   title=f"Shared Space-Loss-{configs.loss_func} \n (Classifier Testing Object)", save_fig=True,
                   subtitle=f"source tool: {configs.source_tool_list}, target tool: {configs.target_tool_list} \n"
                            f"assisted tool: {configs.assist_tool_list}")
...
def viz_embeddings(embeds: np.ndarray, labels: np.ndarray, tool_labels: list, loss_func: str = configs.loss_func,
    ...
    viz_discpt = "T-SNE " if embeds.shape[1] > 2 else "Original "
...
def viz_classifier_boundary_on_2d_embeddings(transfer_class, Encoder, Classifier, accuracy=None):
    ...
    all_emb, all_labels, _ = get_all_embeddings(trans_cls=transfer_class, encoder=Encoder, old_object_list=[])
    ...
    color_label_source, *_ = map_objects_to_colors(object_list, source_y)
    color_label_target, *_ = map_objects_to_colors(object_list, target_y)
    color_label_assist, *_ = map_objects_to_colors(object_list, assist_y)
    ...
    if "assist" in configs.clf_exp_name:
        ...
        plt.scatter(
            ...
            c=color_label_assist, cmap=cmap, norm=norm, edgecolor='gray', s=80, alpha=1.0, marker='X', label="Assist Tool"
        )
