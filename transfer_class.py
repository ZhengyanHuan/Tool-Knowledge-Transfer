...
class Tool_Knowledge_transfer_class:
    ...
    def train_classifier(self, Encoder, behavior_list=configs.behavior_list, trail_list=configs.trail_list,
        ...
        train_encoded_source, val_encoded_source, train_truth_flat, val_truth_flat = self._prepare_data_classifier(
            Encoder=Encoder, behavior_list=behavior_list, source_tool_list=source_tool_list,
    ...
    def eval_classifier(self, Encoder, Classifier, tool_list=configs.target_tool_list,
                        behavior_list=configs.behavior_list, new_object_list=configs.new_object_list,
                        modality_list=configs.modality_list, trail_list=configs.trail_list,
                        return_pred=False) -> Union[Tuple[float, list, list], float]:
    ...
    def train_encoder(self, source_tool_list=configs.source_tool_list, target_tool_list=configs.target_tool_list,
        ...
        """
        hyparams = fill_missing_params(hyparams=hyparams, param_model="encoder")

        logging.debug(f"➡️ train_encoder..")
        loss_record = np.zeros([2, hyparams['epoch_encoder']])
